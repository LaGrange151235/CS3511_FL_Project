...
class server:
    def __init__(self, client_number, num_epoch, ratio, trial):
        self.set_seed()
    ...
    def aggregation_process(self, epoch):
        ...
        while len(client_portion_id_list) < self.portion_number:
            ...
            if (random_id+1) not in client_portion_id_list:
        ...
        for tensor_id, tensor_content in enumerate(self.arrived_tensor_list[client_portion_id_list[0]-1]):
            sum_tensor = torch.zeros(tensor_content.size()).to(self.device)
            for rank in client_portion_id_list:
                sum_tensor += self.arrived_tensor_list[rank-1][tensor_id]
            aggregated_tensor = sum_tensor / len(client_portion_id_list)
...
if __name__=="__main__":
    ...
    server_process = server(args.client_num, args.num_epoch, args.m, args.trial)
